...
def _build_batch_prompt(texts, lang_name):
    ...
    return (
        ...
        f"- Keep proper nouns and acronyms like ICDS, VHSND, Anganwadi unchanged\n"
        f"- Transliterate the word \"creche\" phonetically into the target language's script "
        f"(sound it out in the local script) rather than leaving it in English or forcing a "
        f"literal translation of its meaning -- for example, in Tamil this would be written as "
        f"க்ரீச்\n"
        ...
        f"- If translating a word or phrase (other than \"creche\", per the rule above) would "
        f"spoil or obscure its meaning, leave it in English rather than force a translation\n"
